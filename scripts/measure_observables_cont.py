...
def resonance_pair(omega_i, omega_j, phi_i, phi_j, theta_i, theta_j, theta_a=0, theta_b=0):
    effective_phi_i = phi_i + theta_a
    effective_phi_j = phi_j + theta_b
    cos = math.cos(effective_phi_i - effective_phi_j)
    spin = 1 + math.cos(theta_i + theta_j)  # Continuous spin factor
...
def chsh(omega, phi, theta, sigma):
    i, j = 0, 1
    a, ap = 0.0, math.pi/2
    ...
    for _ in range(N_draw):
        ...
        def E(angle_a, angle_b):
            R = resonance_pair(omega[i], omega_j_noise, phi[i], phi[j], theta[i], theta[j], theta_a=angle_a, theta_b=angle_b)
...
for fn in cfgs:
    dat   = np.load(fn)
    ω, φ, theta = dat['omega'], dat['phi'], dat['theta']
    radii.append(proton_radius(args.links, args.a_eff))
    chshs.append(chsh(ω, φ, theta, args.noise))
